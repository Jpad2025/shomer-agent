...
> Generado automáticamente desde `TECNICO_OPERACION.md` + `SOPORTE_TECNICO.md`.
...
# Versión: mayo 2026 | Idioma: español operacional
...
## Inframonitor — equipos adicionales en vigilancia

Además de los APs y routers que maneja Guardian, el panel tiene un módulo **Inframonitor** para vigilar cualquier otro equipo de red: switches, routers, firewalls, servidores, NAS, cámaras IP, e **impresoras de red**.

### Switches, routers y firewalls con SNMP

Si el equipo tiene SNMP v2c activado, el bot puede decirte mucho más que solo si responde ping:
...
- **Uptime** — cuánto lleva encendido sin reiniciarse
- **Estado de cada puerto**: UP (activo) o DOWN (sin equipo conectado)
- **Tráfico en Mbps** por puerto (requiere 2 lecturas = ~60 segundos)
- **Errores de cable** por puerto — si hay errores, puede ser cable dañado o equipo defectuoso

**Para activar SNMP en un switch/router nuevo:**
1. Entrar al panel web del equipo
2. Activar SNMP v2c
3. Comunidad de lectura: `shomer2026` (o la que indique USB Ingeniería)
4. Restringir acceso: solo IP del Shomer
5. Agregar en el panel Inframonitor con esa comunidad

**Alertas automáticas:** si cualquier equipo Inframonitor cae o se recupera, el bot envía aviso. Pero NO reinicia — solo informa.
...
El bot puede consultar el estado de una impresora en cualquier momento:

> "¿Cómo está la impresora 192.168.1.80?"

Para impresoras el bot reporta: si está en red, si tiene papel, nivel de tóner aproximado (lasers), y si hay un trabajo atascado.

Si una impresora POS (Epson TM-U220 u otra térmica) se desconecta, el bot envía alerta automática con **🖨️ IMPRESORA FUERA DE LÍNEA**.

### Si la impresora deja de imprimir pero el equipo está en red

1. Verificar que hay papel (especialmente en térmicas POS)
2. Si hay trabajos atascados: decirle al bot "limpia la cola de impresión de 192.168.1.X" — él lo hace automáticamente en PCs Windows
3. Si sigue sin imprimir: reiniciar la impresora físicamente
...
| `/modo on` / `/modo off` | Pausar reboots automáticos (`/mantenimiento` igual) |
...
| shomer-agent | activo (bot Telegram) |
...
**Telegram:** aviso al activar y desactivar (`MANTENIMIENTO GLOBAL` + usuario). Desde jun 2026.
...
2. Si no fue suficiente: revisar `/salud` y escalar a ingeniería si persiste.
...
3. **Credenciales:** usuario de dominio AD en Tracker → Credenciales (usuario, contraseña, dominio).
4. **Ficha del equipo:** sección *Monitores* → marcar **Monitor integrado** (portátil/All-in-One) y monitores externos (0–3) con modelo/serial.
...
**Sin `/start`** — entrada: `/consultas`, `/ayuda` o texto libre. Lista completa: `/ayuda`.

| Comando | Para qué sirve |
|---------|----------------|
| `/consultas` | Ejemplos de qué preguntar en texto libre |
| `/ayuda` | Todos los comandos y monitores automáticos |
| `/salud` | Estado del servidor (solo texto, sin botones) |
| `/salud monitores` | Estado de cada monitor automático (26 tareas) |
| `/salud resumen` | Reporte del día con IA |
| `/equipos` | APs Guardian y equipos del agente |
| `/diagnostico 10.10.0.10` | Ping, estado, fallos, uptime |
| `/diagnostico 10.10.0.10 reparar` | Diagnóstico + reparación automática |
| `/reboot 10.10.0.10` | Reiniciar AP — pide confirmación (`/reiniciar` igual) |
| `/modo on` / `/modo off` | Mantenimiento — pausa reboots auto (`/mantenimiento` igual) |
| `/alertas` | Alertas Hunter e IPs bloqueadas |
| `/bloquear 1.2.3.4` | Bloquear IP — con confirmación |
| `/desbloquear 1.2.3.4` | Liberar IP bloqueada |
| `/guardar <IP> <texto>` | Guardar qué pasó para futuras alertas |
| `/historial` | Últimos cambios realizados |
| `/instalar` | Guía de instalación paso a paso |
| `/verificar` | Checklist final de instalación |
| `/nuevo` | Limpiar historial de conversación con la IA |

| `/infra` | Lista equipos Infra |
| `/infra <IP>` | Detalle conexión (ping, TCP, SNMP, tóner) |
| `/puertos <IP>` | Puertos SNMP de switch/router/servidor |

## Lo que el bot hace solo (sin que le pidas)

- AP caído: alerta con botón de reinicio; si hubo solución guardada, la muestra en el mensaje
- AP o equipo Infra recuperado: aviso + opción de guardar qué pasó
- Hunter bloquea IP: alerta; al desbloquear podés guardar falso positivo
- Disco ≥85 %: limpieza automática y aviso
- Servicio Shomer caído: aviso
- Infra: caída, tóner bajo, TCP/SNMP caído, flapping cable/PoE
- Cada mañana 7:00: resumen diario automático
